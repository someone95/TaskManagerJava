		...
		<div id="page-wrapper">
			...
			<div class="panel-body" style="margin-top: 50px;">
				...
				<form class="form-control" style="border: none;"
					action="<%=request.getContextPath()%>/getAllTasks">
					<div class="row">
						<div class="col-md-2">
							Search by:
							<div class="col-md-10">
					...
					<c:if test="${empty itemList.pageList}">
						<h3 align="center">There are no Tasks.</h3>
					</c:if>
								...
								<tbody>
									<c:forEach items="${itemList.pageList}" var="item">
										<tr>
											...
											<th><c:out value="${item.mark}" /></th>
											<th><c:out value="${item.dateOfCreation}" /></th>
											<th><c:out value="${item.dateOfLastEdit}" /></th>
											<th><c:out value="${item.creator.username}" /></th>
											<th><c:out value="${item.assignedUser.username}" /></th>

											<th><a class="text-muted"
												href="<%=request.getContextPath()%>/editTask?id=<c:out value='${item.id}'/>">Edit</a></th>
											<th><a class="text-muted"
												href="<%=request.getContextPath()%>/deleteTask?id=<c:out value='${item.id}'/>">Delete</a></th>
											<th><a class="text-muted"
												href="<%=request.getContextPath()%>/addComment?parentTaskId=<c:out value='${item.id}'/>">Add
													Comment</a></th>
											<th><a class="text-muted"
												href="<%=request.getContextPath()%>/addWorklog?parentTaskId=<c:out value='${item.id}'/>">Add
													Worklog</a></th>

											<th><a class="text-muted"
												href="<%=request.getContextPath()%>/getAllComments?parentId=<c:out value='${item.id}'/>">Show
													Comments</a></th>

											<th><a class="text-muted"
												href="<%=request.getContextPath()%>/getAllWorklogs?parentId=<c:out value='${item.id}'/>">Show
													Worklogs</a></th>
